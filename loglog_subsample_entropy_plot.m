...
for iter=1:20
    ...
    for t=4.^(0:1:4)
       ...
       % Mmax=max(M);
        ...
        Entropy(iter,count) = sum(-(n_k./(sum(n_k))).*log(n_k./(sum(n_k))));
...
subplot(1,4,2);boxplot(UnitRatio);
ax = gca;
ax.XTickLabel = {'1','1/4','1/16','1/64','1/256'};
xlabel('Sampling ratio')
